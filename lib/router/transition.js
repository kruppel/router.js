...
import utils, { trigger, slice, log, promiseLabel } from './utils';
...
function Transition(router, intent, state, error) {
  ...
  if (error) {
    this.promise = utils.Promise.reject(error);
    this.error = error;
    return;
  }
  ...
  if (state) {
    ...
    this.promise = utils.Promise.resolve(this.state);
    this.params = {};
  }

  function checkForAbort() {
    if (transition.isAborted) {
      return utils.Promise.reject(undefined, promiseLabel("Transition aborted - reject"));
    }
  }
}
